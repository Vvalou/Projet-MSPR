...
if ($Mode -eq "Full") {
    ...
    docker exec wms-db mysqldump -uwms_backup -pBackupWMS2025 wms 2>$null > $sqlFile

    if ($LASTEXITCODE -eq 0) {
        Write-LogJson "success" "full_sql" @{ file = $sqlFile }
        Write-Host "[OK] Sauvegarde SQL complete -> $sqlFile"
        exit 0
    }
    else {
        Write-LogJson "error" "full_sql" @{ file = $sqlFile }
        Write-Host "[ERREUR] Echec sauvegarde SQL"
        exit 1
    }
}
...
"@
    ...
    docker exec wms-db mysql -uwms_backup -pBackupWMS2025 wms -e "$sql" 2>$null > $rawFile

    if ($LASTEXITCODE -ne 0) {
        Write-Host "[ERREUR] Erreur lors de l'execution de la requete SQL pour l'export CSV."
    ...
    Write-Host "[OK] Export CSV termine (inventory avec villes + equipements) -> $csvFile"
    exit 0
}
